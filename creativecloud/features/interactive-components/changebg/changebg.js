...
function createContinuePSBtn(data) {
  ...
  continueCTA.addEventListener('click', async (e) => {
    const imgSrc = document.querySelector('.interactive-holder picture img')?.src;
    const { openInPsWeb } = await import('../../../deps/openInPsWeb/openInPsWeb.js');
    const imageData = await (await fetch(imgSrc)).blob();
    openInPsWeb(
      'https://dev.photoshop.adobe.com',
      'filename.jpeg',
      [{ filename: 'filename.jpeg', imageData }],
    );
  });
return continueCTA;
}
